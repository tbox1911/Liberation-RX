	...
	while {true} do {
		{
			if (typeOf _x == commander_classname && isNil {_x getVariable 'GRLIB_FOB_Action'}) then {
				_x addAction ["<t color='#8FFF00'>" + localize "STR_BUILD_DEFENSE" + "</t> <img size='1' image='res\ui_build.paa'/>", "addons\FOB\do_build_def.sqf","",996,false,true,"","call GRLIB_checkPackFOB", 5];
