	...
	while {true} do {
		{
			if (typeOf _x == commander_classname && isNil {_x getVariable 'GRLIB_FOB_Action'}) then {
				...
				_x addAction ["<t color='#8FFF00'>" + localize "STR_BUILD_DEFENSE_SEC" + "</t> <img size='1' image='\a3\Ui_F_Curator\Data\Displays\RscDisplayCurator\modeGroups_ca.paa'/>", "addons\FOB\do_build_sector_def.sqf","",995,false,true,"","GRLIB_player_is_menuok", 5];				
