	...
	while {true} do {
		{
			if (typeOf _x == commander_classname && isNil {_x getVariable 'GRLIB_FOB_Action'}) then {
				...
				_x addAction ["<t color='#8FFF00'>" + localize "STR_BUILD_DEFENSE" + "</t> <img size='1' image='res\ui_build.paa'/>", "addons\FOB\do_build_def.sqf","",997,false,true,"","call GRLIB_checkPackFOB", 5];
				_x addAction ["<t color='#FF8F00'>" + localize "STR_CLEAR_DEFENSE" + "</t> <img size='1' image='res\ui_build.paa'/>", "addons\FOB\do_clean_def.sqf","",996,false,true,"","call GRLIB_checkPackFOB", 5];
				_x addAction ["<t color='#8FFF00'>" + localize "STR_BUILD_DEFENSE_SEC" + "</t> <img size='1' image='\a3\Ui_F_Curator\Data\Displays\RscDisplayCurator\modeGroups_ca.paa'/>", "addons\FOB\do_build_sector_def.sqf","",995,false,true,"","call GRLIB_checkPackFOB", 5];				
