	...
	while {true} do {
		{
			if (typeOf _x == commander_classname && ((_x getVariable ["GRLIB_FOB_Action", -1]) < 0)) then {
				...
				_x addAction ["<t color='#8FFF00'>" + localize "STR_SAVE_PLAYER" + "</t> <img size='1' image='\a3\Ui_f\data\GUI\Rsc\RscDisplayArcadeMap\icon_saveas_ca.paa'/>",{[player,PAR_Grp_ID,false,true] remoteExec ["save_context", 2]},"",993,true,true,"","GRLIB_player_is_menuok",5];
