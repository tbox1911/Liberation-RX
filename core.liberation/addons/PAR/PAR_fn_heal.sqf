...
waitUntil {
	...
	(time > _timer || _medic_busy || _wnded_hit || _medic_hit || _wnded_healed || _medic distance2D _wnded < 3)
};

if (_medic_busy || _wnded_hit || _medic_hit || _wnded_healed) exitWith {
	_wnded stop false;
	_medic setVariable ["PAR_heal", nil];
	_wnded setVariable ["PAR_heal", nil];
};

if (_medic distance2D _wnded <= 3) then {
