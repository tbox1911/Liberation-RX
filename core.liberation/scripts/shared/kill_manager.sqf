...
private [ "_nearby_bigtown", "_msg", "_nearby_smalltown" ];
...
if (isServer) then {
	...
	if (isNil "infantry_weight") then {
		infantry_weight = 33
	};
	if (isNil "armor_weight") then {
		armor_weight = 33
	};
	if (isNil "air_weight") then {
		air_weight = 33
	};
	if (isNil "readiness_calc_bg_town") then {
		readiness_calc_bg_town = false
	};
	if (isNil "readiness_calc_sm_town") then {
		readiness_calc_sm_town = false
	};
	...
	if (side _killer == GRLIB_side_friendly) then {
		if (readiness_calc_bg_town) then {
			_nearby_bigtown = [ sectors_bigtown, {
		...
		if (readiness_calc_sm_town) then {
			_nearby_smalltown = [ sectors_capture, {
				(!(_x in blufor_sectors)) && ( _unit distance (markerPos _x) < 250)
			} ] call BIS_fnc_conditionalSelect;
		...
		if (infantry_weight > 100) then {
			infantry_weight = 100
		};
		if (armor_weight > 100) then {
			armor_weight = 100
		};
		if (air_weight > 100) then {
			air_weight = 100
		};
		if (infantry_weight < 0) then {
			infantry_weight = 0
		};
		if (armor_weight < 0) then {
			armor_weight = 0
		};
		if (air_weight < 0) then {
			air_weight = 0
		};
	};
	...
	if (_unit isKindOf "Man") then {
		...
		if (typeOf _unit == mobile_respawn) exitWith {
			[_unit, "del"] remoteExec ["addel_beacon_remote_call", 2]
		};
