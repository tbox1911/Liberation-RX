...
_infantrycount1 = { alive _x && _x distance2D _position <= _distance && !(captive _x) && isNull objectparent _x && (getPosATL _x) select 2 < 200 } count units _side;
_infantrycount2 = { alive _x && _x distance2D _position <= _distance && _x getVariable ["PAR_Grp_ID", ""] != "" } count units GRLIB_side_civilian;

_countedvehicles =  [ ( _position nearEntities [ ["Car", "Tank", "Air", "Ship"], _distance] ), { !(typeOf _x in uavs) && (getPosATL _x) select 2 < 200 && speed _x <= 100 && count (crew _x) > 0 } ] call BIS_fnc_conditionalSelect;
_vehiclecrewcount = 0;
