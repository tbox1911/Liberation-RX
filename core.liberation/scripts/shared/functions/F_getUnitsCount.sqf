...
_infantrycount = _side countSide ( [ _position nearEntities [ "Man", _distance] , { !(captive _x) && isNull objectparent _x} ] call BIS_fnc_conditionalSelect );
...
_countedvehicles =  [ ( _position nearEntities [ ["Car", "Tank", "Air"], _distance] ), { (getpos _x) select 2 < 300 && speed _x <= 100 && count (crew _x) > 0 } ] call BIS_fnc_conditionalSelect;
_vehiclecrewcount = 0;
