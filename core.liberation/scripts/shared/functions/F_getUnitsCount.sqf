...
if (isNil "air_cannot_trigger_on") then {air_cannot_trigger_on = false};

if (air_cannot_trigger_on) then {
_countedvehicles =  [ ( _position nearEntities [ ["Car", "Tank"], _distance] ), { ((getpos _x) select 2 < 500) && count (crew _x) > 0 } ] call BIS_fnc_conditionalSelect;
} else {
