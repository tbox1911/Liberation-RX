...
private [ "_nextplayer" , "_newscores", "_knownplayers", "_playerindex", "_score", "_ammo", "_fuel" ];
...
while { true } do {
...

	{
		...
		if (_nextplayer getVariable ["GRLIB_score_set", 0] == 0) then {
			...
			// set player rank
			_score = round (_nextplayer getVariable ["GREUH_score_count",0]);
		...
		if (_nextplayer getVariable "GRLIB_score_set" == 1) then {
			...
			if ( _playerindex >= 0 ) then {
				_newscores set [_playerindex, [getPlayerUID _nextplayer, _score, _ammo, _fuel, name _nextplayer]];
			} else {
				_newscores pushback [getPlayerUID _nextplayer, _score, _ammo, _fuel, name _nextplayer];
			};
		};
