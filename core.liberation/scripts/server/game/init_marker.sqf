...
GRLIB_SHOP_Group = createGroup [GRLIB_side_civilian, true];
{
	...
	if (_str find "spe_corner_house_02" > 0) then { _deskDir = (270 +_deskDir); _offset = [4.5, 5, 0.3] };	// SPE
	if (_str find "spe_shop_02" > 0) then { _deskDir = (180 +_deskDir); _offset =  [-1.7, -0.5, 0.7] };		// SPE
