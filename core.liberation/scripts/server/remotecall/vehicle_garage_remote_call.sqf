...
if (_cmd == 1) then {
	...
	GRLIB_virtual_garage append [[typeOf _vehicle, _owner, _color, _ammo, _compo, _lst_a3, _lst_r3f, _lst_grl]];
...
if (_cmd == 2) then {
	...
	GRLIB_virtual_garage deleteAt (_veh_lst select _vehicle);
};

publicVariable "GRLIB_virtual_garage";
sleep 1;
