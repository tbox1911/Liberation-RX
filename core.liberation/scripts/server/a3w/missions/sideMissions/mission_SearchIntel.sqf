...
_setupObjects = {
	...
	// Lamps
	private _lamp = createVehicle ["Land_LampStreet_02_triple_F", zeropos, [], 1, "None"];
	_lamp setPos ([_town_center, 3] call F_getRandomPos);
	_vrac_list pushBack _lamp;
	private _angle = 0;
	for "_i" from 1 to 7 do {
		_nextpos = (_town_center getPos [45, _angle]);
		_lamp = createVehicle ["Land_LampStreet_02_triple_F", _nextpos, [], 1, "None"];
	...
	for "_i" from 1 to 4 do {
		...
		if (count _nextpos > 0) then {
			_dir = floor random 360;
			private _build = createVehicle [selectRandom _buildings, _nextpos, [], 1, "None"];
