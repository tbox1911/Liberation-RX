...
while { GRLIB_endgame == 0 } do {
	...
	if ( combat_readiness >= 80 ) then { _sleeptime = _sleeptime * 0.95 };
	if ( combat_readiness >= 90 ) then { _sleeptime = _sleeptime * 0.90 };
	if ( combat_readiness >= 95 ) then { _sleeptime = _sleeptime * 0.70 };
