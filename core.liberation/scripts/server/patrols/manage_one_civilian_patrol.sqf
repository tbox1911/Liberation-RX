...
while { GRLIB_endgame == 0 && GRLIB_global_stop == 0 } do {
	...
	if ( count _usable_sectors > 0 ) then {
		...
		if (!isNull _civ_grp) then {
			...
			waitUntil {
				sleep 30;
				( GRLIB_global_stop == 1 || (diag_fps < 25) || ({alive _x} count (units _civ_grp) == 0) || (round (speed (leader _civ_grp)) == 0) || (count ([getPos (leader _civ_grp), 4000] call F_getNearbyPlayers) == 0) || (time > _unit_ttl) )
			};
