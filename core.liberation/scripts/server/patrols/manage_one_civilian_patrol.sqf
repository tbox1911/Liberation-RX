...
while { GRLIB_endgame == 0 && GRLIB_global_stop == 0 } do {
	...
	_usable_sectors = [];
	{
		if ( (count ([getmarkerpos _x, GRLIB_spawn_min] call F_getNearbyPlayers) > 0) ) then {
	...
	if ( count _usable_sectors > 0 ) then {
		...
		if (!isNull _civ_grp) then {
			...
			if ( floor(random 100) > 50 ) then {
				_civ_veh = [_sectorpos, (selectRandom civilian_vehicles), false, false, GRLIB_side_civilian] call F_libSpawnVehicle;
			...
			waitUntil {
				...
				(
					...
					([getPos (leader _civ_grp), GRLIB_spawn_max, GRLIB_side_friendly] call F_getUnitsCount == 0) ||
					(time > _unit_ttl)
				)
			};
