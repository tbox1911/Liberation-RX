...
speak_mission_delivery_1 = {
	params ["_unit"];
	if (!(_unit getVariable ["GRLIB_A3W_Mission_SD", false])) exitWith {[_unit] call speak_another_time};
	...
	uIsleep 3;
	_unit globalChat localize "STR_SPEAKMANAGER14";
	uIsleep 3;
...
speak_mission_delivery_2 = {
	params ["_unit"];
	if (isNil "GRLIB_A3W_Mission_Marker") exitWith { [_unit] call speak_another_time };
	if (!(_unit getVariable ["GRLIB_A3W_Mission_SD", false]) || _unit != GRLIB_A3W_Mission_Marker) exitWith { [_unit] call speak_another_time };
	private _next_indx = (GRLIB_A3W_Mission_SD find _unit) + 1;
	private _next_unit = GRLIB_A3W_Mission_SD select _next_indx;
...
speak_mission_delivery_4 = {
	...
	if (!(_unit getVariable ["GRLIB_A3W_Mission_DW", false]) &&
		...
		!(_unit getVariable ["GRLIB_A3W_Mission_DN", false]) ) exitWith { [_unit] call speak_another_time };
