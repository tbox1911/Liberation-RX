...
if (_add) then {
    if (count GRLIB_player_fob_actions == 0) then {
        ...
        // AI Logistics System
        GRLIB_player_fob_actions pushBack (player addAction ["<t color='#0080F0'>" + localize "STR_LOGISTIC_ACTION" + "</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_logistic.sqf","",-985,false,true,"","call GRLIB_checkLogistic"]);
        // continue
        GRLIB_player_fob_actions pushBack (player addAction ["<t color='#0080F0'>" + localize "STR_LOGISTIC_CONTINUE" + "</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_logistic.sqf","",-985,false,true,"CONTINUE","call GRLIB_checkLogisticContinue"]);
        // Build Menu
        ...
        // Destroy Outpost
        GRLIB_player_fob_actions pushBack (player addAction ["<t color='#F02000'>" + localize "STR_DESTROY_OUTPOST" + "</t> <img size='1' image='res\ui_deployfob.paa'/>","scripts\client\actions\do_destroy_outpost.sqf","",-982,false,true,"","call GRLIB_checkDelOutpost"]);
        // Onboard Ship
