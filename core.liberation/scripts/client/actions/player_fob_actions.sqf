...
if (_add) then {
    if (count GRLIB_player_fob_actions == 0) then {
        ...
        // Build Menu
        GRLIB_player_fob_actions pushBack (player addAction ["<t color='#FFFF00'>" + localize "STR_BUILD_ACTION" + "</t> <img size='1' image='res\ui_build.paa'/>","scripts\client\build\open_build_menu.sqf",false,-501,false,true,"","call GRLIB_checkBuild"]);
        // Pack FOB
