...
private _idact_heal=-1;
private _idact_lead=-1;
private _idact_drop=-1;
private _idact_send=-1;
private _idact_secondary=-1;
private _idact_packfob=-1;
private _idact_unpackfob=-1;
private _idact_packtent=-1;
private _idact_unpacktent=-1;
private _idact_buyfuel=-1;
private _idact_admin=-1;
private _idact_options=-1;
private _idact_garage=-1;
private _idact_dog_action1=-1;
private _idact_dog_action2=-1;
private _idact_dog_action3=-1;
private _idact_dog_action4=-1;
private _idact_squad_action1=-1;
private _idact_squad_action2=-1;
private _idact_squad_action3=-1;
private _idact_squad_action4=-1;
	...
	if ([] call is_menuok) then {
		...
		if (GRLIB_enable_arsenal && ( count _near_arsenal != 0 || (player distance lhd) <= 200) ) then {
			if (_idact_arsenal == -1) then {
				_idact_arsenal = player addAction ["<t color='#FFFF00'>" + localize "STR_ARSENAL_ACTION" + "</t> <img size='1' image='res\ui_arsenal.paa'/>","scripts\client\actions\open_arsenal.sqf","",-500,true,true,"","build_confirmed == 0"];
			};
		} else {
			if ( _idact_arsenal != -1 ) then {
		...
		} forEach [ _idact_build,_idact_arsenal,_idact_redeploy,_idact_tutorial,_idact_squad,
					_idact_commander,_idact_repackage,_idact_halo,_idact_heal,_idact_lead,
					_idact_drop,_idact_send,_idact_secondary,_idact_packfob,_idact_unpackfob,
					_idact_packtent,_idact_unpacktent,_idact_buyfuel,_idact_admin,_idact_options,
					_idact_garage,_idact_dog_action1,_idact_dog_action2,_idact_dog_action3,
					_idact_dog_action4,_idact_squad_action1,_idact_squad_action2,_idact_squad_action3,
					_idact_squad_action4 ];
	};
