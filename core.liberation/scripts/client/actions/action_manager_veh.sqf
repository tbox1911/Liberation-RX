...
while { true } do {
...

	{
		...
		if (! (_vehicle getVariable ["GRLIB_vehicle_action", false]) ) then {
			...
			_vehicle addAction ["<t color='#00DD00'>-- SELL CARGO --</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_sell.sqf","",-900,true,true,"","[_target] call is_menuok && [_target] call F_check_nearService && [_this, _target] call is_owner && (locked _target == 0 || locked _target == 1)", _distvehclose];
