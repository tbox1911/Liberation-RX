...
while { true } do {
...

	{
		...
		_vehicle addAction ["<t color='#009F00'>" + localize "STR_REPAIR_VEH" + "</t> <img size='1' image='res\ui_rotation.paa'/>", "scripts\client\actions\do_repair.sqf","",-508,false,true,"","[_target, _this] call GRLIB_checkAction_Repair", GRLIB_ActionDist_10];
	...
	_nearsign = (nearestObjects [player, [FOB_sign], _searchradius]) select { (GRLIB_player_near_lhd || GRLIB_player_near_fob) && isNil {_x getVariable "GRLIB_sign_action"} };
	{
		...
		_unit addAction ["<t color='#FFFFFF'>" + "-= Hall of Fame =-" + "</t> <img size='1' image='\a3\ui_f\data\igui\cfg\simpletasks\types\Talk_ca.paa'/>",{[4] call remote_call_showtext},"",889,false,true,"","GRLIB_player_is_menuok",5];
