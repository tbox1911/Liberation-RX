...
while { true } do {
...

	{
		...
		if (! (_vehicle getVariable ["GRLIB_vehicle_action", false]) ) then {
			...
			_vehicle addAction ["<t color='#00DD00'>-- SELL CARGO --</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_sell.sqf","",-900,true,true,"","[_target] call is_menuok && [_target] call F_check_nearService && [_this, _target] call is_owner", _distvehclose];
			...
			_vehicle addAction ["<t color='#FFFF00'>-- UNFLIP --</t> <img size='1' image='res\ui_flipveh.paa'/>","scripts\client\actions\do_unflip.sqf","",-900,true,true,"","[_target] call is_menuok && [_this, _target] call is_owner", _distveh];
