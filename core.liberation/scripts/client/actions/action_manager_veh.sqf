...
while { true } do {
...

	{
		...
		if (! (_vehicle getVariable ["GRLIB_vehicle_action", false]) ) then {
			...
			_vehicle addAction ["<t color='#00DD00'>-- SELL CARGO --</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_sell.sqf","",-900,true,true,"","[_target] call is_menuok && [_target, 'SRV', _distvehclose, true] call F_check_near && [_this, _target] call is_owner && (locked _target == 0 || locked _target == 1)", _distvehclose];
			...
			_vehicle addAction ["<t color='#FFFF00'>-- UNFLIP --</t> <img size='1' image='res\ui_flipveh.paa'/>","scripts\client\actions\do_unflip.sqf","",-900,true,true,"","[_target] call is_menuok && [_this, _target] call is_owner && (locked _target == 0 || locked _target == 1)", _distveh];
			...
			if (! (typeOf _vehicle in GRLIB_vehicle_blacklist)) then {
				...
				_vehicle addAction ["<t color='#222222'>-- ABANDON --</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_abandon.sqf","",-903,true,true,"","[_target] call is_menuok && [_this, _target] call is_owner && locked _target == 2", _distvehclose];
				_vehicle addAction ["<t color='#444444'>-- CLEAR CARGO --</t> <img size='1' image='res\ui_veh.paa'/>","scripts\client\actions\do_clear.sqf","",-904,true,true,"","[_target] call is_menuok && [_this, _target] call is_owner && locked _target == 2", _distvehclose];
