...
while { true } do {
...

	{
		...
		if (!([typeOf _vehicle, GRLIB_vehicle_blacklist] call F_itemIsInClass) && !([_vehicle] call is_public)) then {
			...
			_vehicle addAction ["<t color='#00F0F0'>" + localize "STR_PAINT" + " (VAM)</t> <img size='1' image='res\ui_veh.paa'/>", "addons\VAM\fn_repaintMenu.sqf","",-905,false,true,"","[_target] call is_menuok && [_target, 'REPAINT'] call F_check_near && [_this, _target] call is_owner && locked _target == 2", _distvehclose];
