...
GRLIB_checkBuildFOB = {
	...
	(GRLIB_player_is_menuok && !GRLIB_player_near_lhd && count (crew _target) == 0 && !(_target getVariable ["box_in_use", false]) && !(_target getVariable ["R3F_LOG_disabled", true]))
};
...
GRLIB_checkLogistic = {
	(GRLIB_player_is_menuok && GRLIB_player_score >= GRLIB_perm_log && GRLIB_player_near_fob && !GRLIB_player_near_outpost && !GRLIB_AI_logistic_in_use && count transport_vehicles > 0)
};
