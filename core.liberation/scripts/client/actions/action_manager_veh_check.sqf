...
GRLIB_checkAction_Halo = {
	params ["_target", "_unit"];
	(GRLIB_player_is_menuok && alive _target && [_unit, _target] call is_owner && [_target, 'FOB', GRLIB_fob_range] call F_check_near && ([_target, ['LandVehicle','Ship']] call F_itemIsInClass) && locked _target < 2)
};
