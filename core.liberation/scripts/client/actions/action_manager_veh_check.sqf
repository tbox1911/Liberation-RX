...
GRLIB_checkAction_Halo = {
	params ["_target", "_unit"];
	([_target] call is_menuok_veh && [_unit, _target] call is_owner && [_target, 'FOB', GRLIB_fob_range] call F_check_near && (_target isKindOf 'LandVehicle' || _target isKindOf 'Ship') && locked _target < 2)
};
