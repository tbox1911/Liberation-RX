...
if (FAC_MSU_ACTIVE) then {
	...
	_comp_build_lists = compile format ["[[], infantry_units, %1_light_vehicles, %1_heavy_vehicles, %1_air_vehicles, static_vehicles, buildings, support_vehicles, squads];
	", _fac];
...
if (( [ getPos player, 500, GRLIB_side_enemy ] call F_getUnitsCount ) > 4) exitWith {
...
while { dialog && alive player && (dobuild == 0 || buildtype == 1) } do {
	...
	if (_oldbuildtype != buildtype || synchro_done) then {
		...
		lbClear 110;
		{
			...
			if(((_x select 1 > 0) && ((_x select 1) > (infantry_cap - resources_infantry))) ||
			((_x select 2 > 0) && ((_x select 2) > _ammo_collected)) ||
			((_x select 3 > 0) && ((_x select 3) > (fuel_cap - resources_fuel)))) then {
				_affordable = false;
			};
	...
	_squad_full = false;
	_ammo_collected = player getVariable ["GREUH_ammo_count", 0];
	_bros = allUnits select {
		(_x getVariable ["PAR_Grp_ID", "0"]) == format["Bros_%1", PAR_Grp_ID]
	};
	_linked = false;
	...
	if (dobuild == 0 && _selected_item != -1 && (_selected_item < (count _build_list))) then {
		...
		if (((_build_item select 1 == 0 ) || ((_build_item select 1) <= (infantry_cap - resources_infantry))) &&
		((_build_item select 2 == 0 ) || ((_build_item select 2) <= _ammo_collected)) &&
		((_build_item select 3 == 0 ) || ((_build_item select 3) <= (fuel_cap - resources_fuel)))) then {
			_affordable = true;
		};
		...
		if (buildtype != 12) then {
			{
				if (( _build_item select 0 ) == ( _x select 0 )) exitWith {
					_base_link = _x select 1;
					_linked = true;
				}
			} forEach GRLIB_vehicle_to_military_base_links;

			if (_linked) then {
				if (!(_base_link in blufor_sectors)) then {
					_linked_unlocked = false
				};
			};
		};
	...
	if (!_linked_unlocked) then {
		_link_color = "#e00000";
		_link_str = localize "STR_VEHICLE_LOCKED";
	};
	if (_linked) then {
