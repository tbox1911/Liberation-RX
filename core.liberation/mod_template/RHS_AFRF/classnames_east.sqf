// *** BADDIES ***
GRLIB_side_enemy = EAST;
GRLIB_color_enemy = "ColorOPFOR";
GRLIB_color_enemy_bright = "ColorRED";

// All class MUST be defined !

opfor_sentry = "CUP_O_sla_Soldier_urban";
opfor_rifleman = "CUP_O_sla_Soldier_urban";
opfor_grenadier = "CUP_O_sla_Soldier_GL_urban";
opfor_squad_leader = "CUP_O_sla_SpecOps";
opfor_team_leader = "CUP_O_sla_SpecOps";
opfor_marksman = "CUP_O_sla_Sniper_urban";
opfor_machinegunner = "CUP_O_sla_Soldier_AR_urban";
opfor_heavygunner = "CUP_O_sla_Soldier_MG_urban";
opfor_medic = "CUP_O_SLA_Soldier_Backpack_urban";
opfor_rpg = "CUP_O_sla_Soldier_AT_urban";
opfor_at = "CUP_O_sla_Soldier_AT_urban";
opfor_aa = "CUP_O_sla_Soldier_AA_urban";
opfor_officer = "CUP_O_Partisans_Soldier_AT";
opfor_sharpshooter = "CUP_O_INS_Sniper";
opfor_sniper = "CUP_O_sla_Soldier_AKS_74_GOSHAWK_urban";
opfor_spotter = "CUP_O_SLA_Spotter_urban";
opfor_engineer = "CUP_O_sla_Engineer_urban";
opfor_pilot = "rhs_vdv_mflora_armoredcrew";
opfor_crew = "rhs_vdv_mflora_armoredcrew";
opfor_paratrooper = "rhs_vdv_mflora_rifleman";
opfor_mrap = "CUP_O_BMP2_SLA";
opfor_mrap_hmg = "CUP_O_BRDM2_SLA";
opfor_mrap_gmg = "CUP_O_GAZ_Vodnik_AGS_RU";
opfor_transport_helo = "CUP_O_Mi24_D_Dynamic_SLA";
opfor_transport_truck = "CUP_O_Ural_Open_SLA";
opfor_fuel_truck = "CUP_O_Ural_Refuel_CHDKZ";
opfor_ammo_truck = "CUP_O_Ural_Reammo_CHDKZ";
opfor_fuel_container = "Land_Pod_Heli_Transport_04_fuel_F";
opfor_ammo_container = "Land_Pod_Heli_Transport_04_ammo_F";
opfor_house = "Land_Cargo_House_V3_F";
opfor_patrol = "Land_Cargo_Patrol_V3_F";
opfor_hq = "Land_Cargo_HQ_V3_F";

militia_squad = [
    "CUP_O_Partisans_Soldier_MG",
    "CUP_O_Partisans_Medic",
    "CUP_O_Partisans_Soldier_AT",
    "CUP_O_Partisans_soldier_SL",
    "CUP_O_Partisans_Engineer",
    "CUP_O_Partisans_soldier_TTsKO",
	"CUP_O_sla_Soldier_LAT",
	"CUP_O_sla_Sniper",
	"CUP_O_SLA_Sniper_SVD_Night",
	"CUP_O_SLA_Sniper_SVD_Night",
	"CUP_O_sla_Soldier_AKS_Night",
	"CUP_O_sla_Soldier_AKS_Night",
	"CUP_O_sla_Soldier_GL",
	"CUP_O_sla_Soldier_GL"
];

militia_loadout_overide = [
];

divers_squad = [
	"O_diver_TL_F",
	"O_diver_TL_F",
	"O_diver_exp_F",
	"O_diver_exp_F",
	"O_diver_exp_F",
	"O_diver_exp_F",
	"O_diver_F",
	"O_diver_F",
	"O_diver_F",
	"O_diver_F",
	"O_diver_F",
	"O_diver_F",
	"O_diver_F",
	"O_diver_F"
];

militia_vehicles = [
    "CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_MG_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_UAZ_Militia_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_BRDM2_SLA",
	"CUP_O_Ural_ZU23_SLA",
	"CUP_O_ZSU23_SLA",
	"CUP_O_UAZ_AA_SLA",
	"CUP_O_MTLB_pk_SLA",
	"CUP_O_BTR80_SLA",
    "CUP_O_BMP2_SLA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_BTR80A_CHDKZ",
	"CUP_O_T55_CHDKZ",
	"CUP_O_T55_CHDKZ"
];

boats_east = [
	"O_Boat_Armed_01_hmg_F"
];

opfor_vehicles = [
    "CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_MG_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_UAZ_Militia_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_BRDM2_SLA",
	"CUP_O_Ural_ZU23_SLA",
	"CUP_O_ZSU23_SLA",
	"CUP_O_UAZ_AA_SLA",
	"CUP_O_MTLB_pk_SLA",
	"CUP_O_BTR80_SLA",
    "CUP_O_BMP2_SLA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_BTR80A_CHDKZ",
	"CUP_O_T55_CHDKZ",
	"CUP_O_T55_CHDKZ",
	"B_SAM_System_02_F"
];

opfor_vehicles_low_intensity = [
    "CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_MG_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_UAZ_Militia_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_BRDM2_SLA",
	"CUP_O_Ural_ZU23_SLA",
	"CUP_O_ZSU23_SLA",
	"CUP_O_UAZ_AA_SLA",
	"CUP_O_MTLB_pk_SLA",
	"CUP_O_BTR80_SLA",
    "CUP_O_BMP2_SLA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_BTR80A_CHDKZ",
	"CUP_O_T55_CHDKZ"
];

opfor_battlegroup_vehicles = [
    "CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_MG_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_UAZ_Militia_SLA",
	"CUP_O_UAZ_SPG9_SLA",
	"CUP_O_BRDM2_SLA",
	"CUP_O_Ural_ZU23_SLA",
	"CUP_O_ZSU23_SLA",
	"CUP_O_UAZ_AA_SLA",
	"CUP_O_MTLB_pk_SLA",
	"CUP_O_BTR80_SLA",
    "CUP_O_BMP2_SLA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_T34_TKA",
	"CUP_O_BTR80A_CHDKZ",
	"CUP_O_T55_CHDKZ",
	"CUP_O_2S6_RU"
];

opfor_battlegroup_vehicles_low_intensity = [
	"CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_MG_SLA",
	"CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_AGS30_SLA"
];

opfor_reinforcement_vehicles = [
	"CUP_O_GAZ_Vodnik_AGS_RU",
	"CUP_O_GAZ_Vodnik_AGS_RU",
	"CUP_O_UAZ_AGS30_SLA",
	"CUP_O_UAZ_MG_SLA",
	"CUP_O_T34_TKA",
	"CUP_O_T55_TK",
	"CUP_O_BRDM2_SLA",
	"CUP_O_Ural_ZU23_SLA",
	"CUP_O_BMP2_SLA",
	"CUP_O_BMP3_RU",
	"CUP_O_2S6_RU",
	"CUP_O_ZSU23_SLA"
	];

opfor_troup_transports = [
	"CUP_O_BTR60_CHDKZ",
	"CUP_O_BTR60_CHDKZ",
	"CUP_O_BMP2_ZU_TKA",
	"CUP_O_BMP2_ZU_TKA",
	"RHS_Mi24P_vdv"
];

opfor_choppers = [
	"CUP_O_UH1H_gunship_SLA",
	"CUP_O_Mi24_P_Dynamic_RU",
	"CUP_O_Ka50_DL_SLA",
	"CUP_O_UH1H_gunship_SLA",
	"CUP_O_UH1H_gunship_SLA",
	"CUP_O_Ka50_DL_SLA",
	"CUP_O_Mi24_D_Dynamic_SLA"
];

opfor_air = [
	"CUP_O_Ka52_RU",
	"rhs_mi28n_vvsc",
	"CUP_O_SU34_RU",
	"CUP_O_SU34_RU",
	"rhs_mig29sm_vvs",
	"FIR_Su25SM3",
	"rhs_mig29sm_vvs",
	"FIR_Su25SM3",
	"RHS_T50_vvs_generic_ext",
	"FIR_Su25SM3",
	"RHS_T50_vvs_generic_ext",
	"O_Plane_Fighter_02_F",
	"O_Plane_Fighter_02_F"
];

opfor_statics = [
	"CUP_O_Igla_AA_pod_ChDKZ",
	"B_SAM_System_02_F"
];

opfor_recyclable = [
	["B_UAV_06_F",0,0,0],
	["B_UAV_06_medical_F",0,0,0],
	["B_UAV_01_F",0,0,0],
	["ITC_Land_B_UAV_AR2e",0,0,0],
	["ITC_Land_B_UAV_AR2i",0,0,0],
	["B_UGV_02_Demining_F",0,0,0],
	["B_UGV_02_Science_F",0,0,0],
	["O_UGV_02_Demining_F",0,0,0],
	["ITC_Land_O_UAV_AR2e",0,0,0],
	["ITC_Land_O_UAV_AR2i",0,0,0],
	["O_UAV_06_F",0,0,0],
	["O_UAV_06_medical_F",0,0,0],
	["O_UGV_02_Science_F",0,0,0],
	["O_UAV_01_F",0,0,0],
	["I_UAV_06_F",0,0,0],
	["I_UAV_06_medical_F",0,0,0],
	["I_UAV_01_F",0,0,0],
	["ITC_Land_I_UAV_AR2e",0,0,0],
	["ITC_Land_I_UAV_AR2i",0,0,0],
	["I_UGV_02_Demining_F",0,0,0],
	["I_UGV_02_Science_F",0,0,0],
	["I_E_UAV_06_F",0,0,0],
	["I_E_UAV_06_medical_F",0,0,0],
	["I_E_UAV_01_F",0,0,0],
	["I_E_UGV_02_Demining_F",0,0,0],
	["I_E_UGV_02_Science_F",0,0,0],
	["C_UAV_06_F",0,0,0],
	["C_UAV_06_medical_F",0,0,0],
	["C_IDAP_UAV_06_antimine_F",0,0,0],
	["C_IDAP_UGV_02_Demining_F",0,0,0],
	["C_IDAP_UAV_01_F",0,0,0],
	["C_IDAP_UAV_06_F",0,0,0],
	["C_IDAP_UAV_06_medical_F",0,0,0]
];




// loot crates for logistics 

loot_crates = [

	["Box_NATO_AmmoVeh_F", 0, round(box_recycle_value / GRLIB_recycling_percentage), 0],
	["Box_East_AmmoVeh_F", 0, round(box_recycle_value / GRLIB_recycling_percentage), 0],
	["Box_IND_AmmoVeh_F", 0, round(box_recycle_value / GRLIB_recycling_percentage), 0],
	["Land_BarrelWater_F", 0, round(box_recycle_value / GRLIB_recycling_percentage), 0],
	["Land_FoodSacks_01_large_brown_idap_F", 0, round(box_recycle_value / GRLIB_recycling_percentage), 0],
	["Land_MetalBarrel_F", 0, round(box_recycle_value / GRLIB_recycling_percentage), 0]

];

box_transport_config_east = [
	 // [ "CUP_B_MTVR_BAF_DES", -6.5, [0, -0.4, 0.3], [0, -2.1, 0.3] ],  
	[ "CUP_O_V3S_Covered_TKA", -5.5, [0, 0.3, 0], [0, -1.25, 0] ]
];



