...
while {true} do
{
	...
	) then
	{
		...
		if (_fonctionnalites select __can_be_moved_by_player) then
		{
			...
			R3F_LOG_action_deplacer_objet_valide = alive _objet_pointe &&
				(count crew _objet_pointe == 0 || _isUav || typeOf _objet_pointe in static_vehicles_AI) && (isNull R3F_LOG_joueur_deplace_objet) &&
		...
		if (_fonctionnalites select __can_be_transported_cargo) then
		{
			...
			if (_fonctionnalites select __can_be_moved_by_player) then
			{
				...
				R3F_LOG_action_charger_deplace_valide = alive _objet_pointe &&
					(count crew _objet_pointe == 0 || _isUav) && (R3F_LOG_joueur_deplace_objet == _objet_pointe) &&
			...
			R3F_LOG_action_selectionner_objet_charge_valide = alive _objet_pointe &&
				(count crew _objet_pointe == 0 || _isUav) && isNull R3F_LOG_joueur_deplace_objet &&
	...
	) then
	{
		...
		R3F_LOG_action_heliport_larguer_valide = !isNull (_vehicule_joueur getVariable "R3F_LOG_heliporte") && !(_vehicule_joueur getVariable "R3F_LOG_disabled") &&
			(vectorMagnitude velocity _vehicule_joueur < 25) && ((getPosASL _vehicule_joueur select 2) - (0 max getTerrainHeightASL getPos _vehicule_joueur) <= 25);

		// Condition action heliport_paradrop
		R3F_LOG_action_heliport_paradrop_valide = !isNull (_vehicule_joueur getVariable "R3F_LOG_heliporte") && !(_vehicule_joueur getVariable "R3F_LOG_disabled") &&
			(vectorMagnitude velocity _vehicule_joueur < 25) && ((getPosASL _vehicule_joueur select 2) - (0 max getTerrainHeightASL getPos _vehicule_joueur) >= 200);
	}
	else
	{
