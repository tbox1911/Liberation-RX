...
R3F_LOG_FNCT_3D_tirer_position_degagee_sol = {
	...
	for [
		...
		{
			if (!_eau_autorise && surfaceIsWater _position_degagee) then {
				_nb_tirages < _nb_tirages_max;
			} else {
				_intersect = false;
...
R3F_LOG_FNCT_3D_get_objets_genants_rayon = {
	...
	_obj_proches = [];
	{
		...
		if (_volume > 0.08) then
		{
			...
			if (!(_x isKindOf "Animal_Base_F") && !(typeOf _x in ["HouseFly", "HoneyBee", "Mosquito"])) then { _obj_proches pushBack _x };
		};
	...
	_elements_terrain = [];
	{
		...
		if (vectorMagnitude velocity _e == 0) then
		{
			...
			{
				// Filtre : insectes et vie ambiante
				if (!(_x isKindOf "Animal_Base_F") && !(typeOf _x in ["HouseFly", "HoneyBee", "Mosquito"])) then {
