		...
		if (isNull (_objet getVariable ["R3F_LOG_remorque", objNull])) then
		{
			if (count crew _objet == 0 || getNumber (configOf _objet >> "isUav") == 1 || typeOf _objet in static_vehicles_AI) then
			{
				...
				if (_objet getVariable "R3F_LOG_est_deplace_par" == _joueur && isNull (_objet getVariable ["R3F_LOG_est_transporte_par", objNull])) then
				{
					...
					_action_relacher = _joueur addAction [("<t color=""#ee0000"">" + format [STR_R3F_LOG_action_relacher_objet, [_objet] call F_getLRXName] + "</t>  <img size='1' image='R3F_LOG\icons\r3f_release.paa'/>"), {_this call R3F_LOG_FNCT_objet_relacher}, nil, 10, true, true];
